...
function exec(args) {
  ...
  args.solidity_file_paths.forEach((solidity_file_path) => {    
      ...
      const structMembers = targetStruct.members.map((targetMember) => {
        ...
        switch (targetMember.typeName.type) {
          case 'UserDefinedTypeName':
            varKind = 'struct';
            varByteSize = 0; // handled in loop after structs.reduce
            varNestedStructName = targetMember.typeName.namePath;
            break;
          case 'Mapping':
            varKind = 'mapping';
            varByteSize = 32;
    ...
    // for each nested struct check how many storage slots (=32 bytes) it occupies (recursively)
    Object.keys(parsedStructs).forEach((contractName) => {
      Object.keys(parsedStructs[contractName]).forEach((structName) => {
        console.log(JSON.stringify(parsedStructs[contractName][structName], null, 2));
        parsedStructs[contractName][structName].forEach((structMember, idx) => {
          if (structMember.varByteSize != 0) return;
          // NOTE: we assume this is a flattened file and struct is in same contract
          // TODO: allow struct to also be in another contract in this same file
          const foundStruct = parsedStructs[contractName][structMember.varNestedStructName];
          if (!foundStruct) {
            parsedStructs[contractName][structName][idx].varByteSize = 32;
            return; // NOTE: must be an enum
          }
          const foundStructByteSize = foundStruct.map(member => member.varByteSize).reduce((acc, v) => acc + v, 0);
          const storageSlots = Math.ceil(foundStructByteSize / 32); // we want multiple of 32 bytes, rounded up
          parsedStructs[contractName][structName][idx].varByteSize = storageSlots * 32; 
        });
      });
    });
      ...
      Object.keys(parsedStructs[contractName]).forEach((structName) => {
        ...
        parsedStructs[contractName][structName].forEach((targetMember) => {
          if (targetMember.varByteSize + storageSlotByteCount > 32) {
            ...
            if (targetMember.varByteSize > 32) { // can only be nested structs
              console.log(`  ${input.split('\n')[targetMember.varLine].replace(/\/\/.+/, '').trim()} // bytes: ${targetMember.varByteSize}`);
              const structStorageSlots = targetMember.varByteSize / 32;
              [...Array(structStorageSlots)].map((_, i) => {
                console.log('  //' + '-'.repeat(10) + ` end of slot ${slotCount} | bytes taken: 32 | bytes free: 0`);
                slotCount += 1;
              });
              console.log();
              storageSlotByteCount = 0;        
              return;
            }
          } else {
            storageSlotByteCount += targetMember.varByteSize;
    ...
    Object.keys(inefficientStructs[inputFilePath]).forEach((contractName) => {
      Object.keys(inefficientStructs[inputFilePath][contractName]).forEach((structName) => {
        ...
        if (structSummary.current > structSummary.optimized) {
          ...
          console.log(`// slots saved: ${structSummary.current - structSummary.optimized}`);
          console.log('// -----------------------------');
          if (args.output_json_path) {
            jsonOutput.push({
              ...
              struct: structName,              
              currentSlots: structSummary.current,
              optimizedSlots: structSummary.optimized,
              numSlotsSaved: structSummary.current - structSummary.optimized, 
            });
          }
        }
      });
